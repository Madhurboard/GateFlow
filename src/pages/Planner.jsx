            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 glass-card p-6">
                    <div className="flex items-center justify-between mb-5">
                        ...
                        <div className="flex items-center gap-2">
                            <button onClick={() => setWeekOffset(o => o - 1)} className="p-1.5 rounded-lg hover:bg-slate-50 text-slate-400"><ChevronLeft size={18} /></button>
                            <span className="text-xs font-bold text-slate-500 min-w-[100px] text-center">{weekDays[0].month} {weekDays[0].date} – {weekDays[6].date}</span>
                            <button onClick={() => setWeekOffset(o => o + 1)} className="p-1.5 rounded-lg hover:bg-slate-50 text-slate-400"><ChevronRight size={18} /></button>
                        </div>
                    </div>
                    <div className="grid grid-cols-7 gap-2">
                        {weekDays.map((day, di) => (
                            <div key={di} className="min-h-[200px]">
                                <div className={`text-center py-2 mb-2 rounded-lg ${day.isToday ? 'bg-primary text-white' : 'bg-slate-50 text-slate-500'}`}>
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <button onClick={() => openAddSession(di)} className="w-full py-2 text-slate-300 hover:text-primary hover:bg-primary/5 rounded-lg transition-colors text-xs">
                ...
                <div className="glass-card p-6 flex flex-col">
                    ...
                    <div className="flex-1 space-y-1">
                        {tasks.map(task => (
                            <div key={task.id} className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-slate-50 transition-colors group">
                                <div
                                    onClick={() => toggleTask(task.id)}
                                    className={`w-5 h-5 rounded-md border-2 flex-shrink-0 flex items-center justify-center cursor-pointer transition-all ${task.done ? 'bg-primary border-primary' : 'border-slate-200 group-hover:border-slate-300'
                                        }`}
                                >
                                    {task.done && <Check size={14} className="text-white stroke-[3px]" />}
                                </div>
                                <span className={`text-sm flex-1 ${task.done ? 'text-slate-400 line-through' : 'text-slate-600'}`}>{task.text}</span>
                    ...
                    <div className="mt-3 pt-3 border-t border-slate-100">
                        <div className="flex gap-2">
                            <input
                                ...
                                className="flex-1 text-sm bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all"
                            />
            ...
            <AnimatePresence>
                {showModal && editingSession && (
                    ...
                    >
                        ...
                        >
                            <div className="flex items-center justify-between mb-5">
                                <h3 className="text-lg font-bold text-slate-800">{editingSession.id ? 'Edit Session' : 'Add Study Session'}</h3>
                                <button onClick={() => setShowModal(false)} className="text-slate-400 hover:text-slate-600"><X size={20} /></button>
                            </div>
                            <div className="space-y-4">
                                <div>
                                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1">Day</label>
                                    <select value={editingSession.day} onChange={e => setEditingSession({ ...editingSession, day: +e.target.value })} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-primary/30">
                            ...
                            <div className="flex gap-3 mt-6">
                                ...
                                <button onClick={() => setShowModal(false)} className="flex-1 px-4 py-2.5 bg-slate-100 text-slate-600 rounded-xl text-sm font-semibold hover:bg-slate-200 transition-colors">Cancel</button>
